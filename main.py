...
def start_process():
    ...
    if hashlib.md5(key.encode('utf-8')).hexdigest() != md5_hash:
        messagebox.showinfo("WRONG PASSWORD", "The password you entered is incorrect. Please try again.")
        return False
    try:
        if action == "Encrypt":
...
style.configure("TLabel", background="#1e1e2f", foreground="#ffffff", font=("Helvetica", 11))
...
ttk.Entry(frame, textvariable=password_var, show="*").grid(row=1, column=1, columnspan=2, sticky="we", padx=5)

# Action
tk.Label(frame, text="Action:").grid(row=2, column=0, sticky="w", pady=10)
ttk.OptionMenu(frame, action_var, "Encrypt", "Encrypt", "Decrypt").grid(row=2, column=1, sticky="w", padx=5)
...
# Footer
tk.Label(frame, text="© 2025 Secure Media Tools", font=("Helvetica", 9), fg="#aaaaaa", bg="#1e1e2f").grid(row=4, column=0, columnspan=3, pady=5)
